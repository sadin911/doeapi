...
path_input = r'/home/chonlatid/py_project/4corner/images/input/*.jpg'
bg_path = r'/home/chonlatid/py_project/4corner/images/background/*.jpg'
save_path = r'/home/chonlatid/py_project/4corner/images/output'
...
#            img3 = cv2.resize(img3, dsize=(self.input_shape[0], self.input_shape[1]), interpolation=cv2.INTER_LANCZOS4)
        ...
        y_topleft = ind[0] /array_image[0].shape[0]
        x_topleft = ind[1] /array_image[0].shape[1]
        ind = np.unravel_index(np.argmax(array_image[1], axis=None), array_image[1].shape)
        y_bottomleft = ind[0]/array_image[0].shape[0]
        x_bottomleft = ind[1]/array_image[0].shape[1]
        ind = np.unravel_index(np.argmax(array_image[2], axis=None), array_image[1].shape)
        y_topright = ind[0]/array_image[0].shape[0]
        x_topright = ind[1]/array_image[0].shape[1]
        ind = np.unravel_index(np.argmax(array_image[3], axis=None), array_image[1].shape)
        y_bottomright = ind[0]/array_image[0].shape[0]
        x_bottomright = ind[1]/array_image[0].shape[1]
    ...
    def plot_corner(self,img,target,color = (255,0,0)):
        ...
        draw.polygon([(target[0]*shape[1],target[1]*shape[0]),(target[2]*shape[1],target[3]*shape[0]),(target[4]*shape[1],target[5]*shape[0]),(target[6]*shape[1],target[7]*shape[0]) ],  outline=color)
...
#        draw.polygon([(target[0],target[1]),(target[2],target[3]),(target[4],target[5]),(target[6],target[7]) ],  outline=color)
    ...
    def viz_result(self,folder_name, img_count):
        ...
        for i in range(img_count):
            ...
            viz_img = self.plot_corner(img,predict_corner)
...
#                sys.stdout.write(ERASE_LINE)
                ...
                predict_corner = self.model.predict(indput_data)[0]
                viz_img = self.plot_corner(img_viz,predict_corner)
                try:
...
#                    self.model.save_weights(os.path.join(save_path,'weight','epoch_' + str(epoch) + '.h5'))
            ...
            self.model.save_weights('step5665.h5')
...
if __name__ == '__main__':
    cor = corner()
    #cor.model.load_weights(r'step5665.h5')
#    cor.viz_result('test',6)
    cor.train(0,100,1,10)
