...
# image path
KERAS_REST_API_URL = r"http://0.0.0.0:8080/api/docscanner"

# load the input image and construct the payload for the request
file_list = sorted(glob.glob(r'/media/trainai/data/db/namelistOCR/original/*'))
timelist = []
for i in range(len(file_list)):
    ...
    imgsave = os.path.join('/media/trainai/data/db/namelistOCR/processed' , os.path.basename(file_list[i])[0:-4] + str('zc.jpg'))
    vizsave = os.path.join('jsonfromAPI' , os.path.basename(file_list[i])[0:-4] + str('in.jpg'))
    with open(filesave, 'w') as data_file:
        json.dump(out, data_file)
        image.save(imgsave)
        vizin.save(vizsave)
